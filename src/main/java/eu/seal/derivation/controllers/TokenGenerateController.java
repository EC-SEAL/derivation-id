...
import eu.seal.derivation.enums.TypeEnum;
import eu.seal.derivation.model.pojo.AttributeSet;
import eu.seal.derivation.model.pojo.AttributeSetStatus;
...
public class TokenGenerateController {
	 ...
	 * Redirects an existing IDP request to the IDP 
	 */	
	@RequestMapping(value = "generate/storeAuthSet", method = {RequestMethod.POST, RequestMethod.GET}, produces = "application/json")
	@ResponseBody
	public String storeAuthSet(@RequestParam(value = "session", required = true) String sessionID, @RequestParam(value = "sender", required = true) String sender, @RequestParam(value = "receiver", required = true) String receiver,RedirectAttributes redirectAttrs) throws KeyStoreException, NoSuchAlgorithmException, IOException {
		String sessionMngrUrl = System.getenv("SESSION_MANAGER_URL");
		...
		String attributeSetString = mapper.writeValueAsString(dataReceived);
		...
		UpdateDataRequest updateReqAuthSet = new UpdateDataRequest(sessionID, "authenticationSet", attributeSetString);
		String rsp = netServ.sendPostBody(sessionMngrUrl, "/sm/updateSessionData", updateReq, "application/json", 1);
		String rspAuthSet = netServ.sendPostBody(sessionMngrUrl, "/sm/updateSessionData", updateReqAuthSet, "application/json", 1);
		return rsp;		
	}
